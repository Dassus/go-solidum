...
// Package utils contains internal helper functions for go-solidum commands.
package utils
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		urls = params.SolidumTestnetBootnodes
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	case ctx.GlobalBool(TestnetFlag.Name):
		...
		cfg.Genesis = core.DefaultSolidumTestnetGenesisBlock()
	case ctx.GlobalBool(RinkebyFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 4
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	switch {
	case ctx.GlobalBool(TestnetFlag.Name):
		genesis = core.DefaultSolidumTestnetGenesisBlock()
	case ctx.GlobalBool(RinkebyFlag.Name):
